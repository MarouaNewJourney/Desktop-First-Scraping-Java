...
public void insertionDataScrapping(Connection con1,int nbrcompagnie,int nbrAnnonce){
		...
		try {
		    ...
		    String data ="INSERT  INTO `datascraping` (`date_scraping`, `nbr_compagnie`,`nbr_annonce`) VALUES("+"'"+java.time.LocalDate.now()+"'"+","+"'"+nbrcompagnie+"'"+","+"'"+nbrAnnonce+"'"+");";
...
public  void deleteRegression(Connection con1){
	
	try {
	
	
	
		String sql1 = "DELETE from  `regression`  ";
		 Statement smt1= con1.createStatement();
		 int rs1 = smt1.executeUpdate(sql1);
