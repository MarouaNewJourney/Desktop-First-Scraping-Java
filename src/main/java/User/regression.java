...
public class regression extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					regression frame = new regression();
	...
	public regression() {
		...
		try {
				String sql="SELECT * FROM regression";
				Statement smt = con.createStatement();
				ResultSet rs = smt.executeQuery(sql);
				
				while (rs.next())
				{
					model.addRow(new Object[] {rs.getString("domaine"), rs.getInt("nbr_annonces"), rs.getInt("nbr_postes"), rs.getInt("nbr_compagnies")});
				}
		...
		sl_contentPane.putConstraint(SpringLayout.WEST, pane, 163, SpringLayout.EAST, btnNewButton_5);
		sl_contentPane.putConstraint(SpringLayout.EAST, pane, 466, SpringLayout.EAST, btnNewButton_5);
